...
sufijo `_l` aquí denota "definición alternativa/legado de recursión", **no** la
variante lateral `_left` de la Regla 11 — no confundir ambos usos. Este patrón no
está declarado en ningún ADR; revisar si `add_l`/`add_zero_l`/`zero_add_l` deberían
marcarse `private` dado que son andamiaje interno, no API pública pensada para
consumirse desde fuera.
