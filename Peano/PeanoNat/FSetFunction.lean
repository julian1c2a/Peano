...
License: MIT
  ...
  4] Más aún, deberíamos poder generalizar MapOn no solo a dos conjuntos finitos sino a una familia finita de ellos.
     ...
     4.1] lexLt y lexLe para Tuple n
          Serán ordenes lexicográficos sobre tuplas de naturales
  5] FSetList: def FSetList := List ℕ₀FSet
  ...
  6] lexListFsetLt y lexListFsetLe para FSetList
     Serán órdenes lexicográficos sobre listas de conjuntos finitos de naturales
     6.1.] Primero por el cardinal, un conjunto es mas pequeño que otro si el cardinal es menor
     6.2.] Si tienen el mismo cardinal, entonces se comparan lexicográficamente entre sí
  7] FSetSet:
   ...
   8] Nos hace falta un tipo tupla de conjuntos finitos de naturales, parametrizado por un natural
      inductive FSetTuple : ℕ₀ → Type where
        | 𝟘 : FSetTuple 0
        | σ n : ℕ₀FSet × FSetTuple n
   9] Tenemos ya conjuntos de FSet y tipos tupla de n FSet, pero nos falta un tipo de función entre ellos. Podríamos definir algo como:
     structure FSetFunction (n m : ℕ₀) where
       toFun : FSetTuple n → FSetTuple m
       map_underset : ∀ t, t ∈ FSetTuple n → toFun t ∈ FSetTuple m
