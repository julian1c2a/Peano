...
theorem eq_of_max_eq_min(n m : ℕ₀) :
    (max n m = min n m) → (n = m)
        := by
...
theorem eq_iff_eq_max_min(n m : ℕ₀) :
    n = m ↔ max n m = min n m
        ...
        · -- Caso max n m = min n m → n = m
          ...
          exact eq_of_max_eq_min n m h_hyp_max_eq_min
...
theorem max_ne_min_of_ne(n m : ℕ₀) :
    n ≠ m ↔
    ((max n m = n) ∧ ¬(max n m = m))
    ∨
    (¬(max n m = n) ∧ (max n m = m))
