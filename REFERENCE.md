...
* **`lt_of_lt_of_le {a b c : ℕ₀} (h_lt_ab : Lt a b) (h_le_bc : Le b c) : Lt a c`**: Transitividad mixta: si `a < b` y `b ≤ c`, entonces `a < c`.

* **`mul_sub (c a b : ℕ₀) (h_lt : Lt b a) : c * (a - b) = c * a - c * b`**: Distributividad de la multiplicación sobre la resta truncada, bajo la condición `b < a`.
