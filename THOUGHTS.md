...
*$\subseteq$*: Si $k \in N(H\cap M) \cap (H\cap K)$, escribimos $k = nm$ con $n \in N$, $m \in H\cap M$, $k \in H \cap K$. Como $m \in M \leq K$ y $k \in K$, se tiene $n = km^{-1} \in K$; como además $n \in N$, concluimos $n \in N \cap K$, y por tanto $k = \underbrace{n}_{\in N\cap K} \cdot \underbrace{m}_{\in H\cap M} \in (N\cap K)(H\cap M)$. ✓
...
*$\subseteq$*: Sea $k = mn$ con $m \in M$, $n \in N\cap K$, $k \in H\cap K$. Como $n \in N \leq H$ y $k \in H$, tenemos $m = kn^{-1} \in H$; como $m \in M$, resulta $m \in H\cap M$. Además $m \in M \leq K$ y $n \in K$, así que $m \in H\cap K$. Por la igualdad $AB = BA$ del Paso 3a aplicada en $L = H\cap K$, $mn \in (N\cap K)(H\cap M)$. ✓
