...
### [2] Renombrar `MaxMin.lean` → `Lattice.lean` y ampliar hacia estructura de retículo

**Situación actual:** `MaxMin.lean` define `max`, `min`, `min_max` y teoremas de
...
1. Renombrar `MaxMin.lean` → `Lattice.lean` (fase 1: solo rename).
...
2. **Rename `MaxMin` → `Lattice`** y ampliar la estructura de retículo.
